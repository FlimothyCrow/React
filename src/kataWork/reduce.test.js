import * as reduce from "./reduce.js"; // ./ means "this directory"

test("reduceFun0", () => {
  var listOfInts = [1, 2];
  var seed = 0;
  var x = reduce.reduceFun(listOfInts, (a, b) => a + b, seed);
  expect(x).toBe(3);
});
// ------------------
test("reduceFun1", () => {
  ...
  var seed = "";
  var x = reduce.reduceFun(listOfThings, (a, b) => a + b, seed);
...
// ------------------
test("reduceFun2", () => {
  // lambda with no parameters to stop it from running until test executes
  var listOfThings = ["ferzz", "bolg", "bolg", "bolg"]; // JS is lambda af
  var seed = {};
  var x = reduce.reduceFun(
    listOfThings,
    (acc, next) => {
      // arguments are part of lambda parameter
      if (acc[next]) {
        acc[next] += 1;
      } else {
        acc[next] = 1;
...
test("longReduce0", () => {
  var listOfInts = [1, 2];
  var starter = 0;
  var x = reduce.reduceFun(listOfInts, (a, b) => a + b, starter);
  expect(x).toBe(3);
});
...
test("doubleSort", () => {
  ...
  var x = reduce.doubleSort(arrayOfObjects);
...
test("reducePractice", () => {
  let x = reduce.sumPlusTen([1, 2, 3]);
  expect(x).toBe(16); // first and second
});
